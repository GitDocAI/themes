...
interface CarouselEditModalProps {
  isOpen: boolean
  onClose: () => void
  onUpdate: (images: any[], numVisible: number, circular: boolean) => void
  onUpload: (file: File) => Promise<string>
  initialImages?: any[]
  initialNumVisible?: number
  initialCircular?: boolean
}

export const CarouselEditModal: React.FC<CarouselEditModalProps> = ({
  isOpen,
  onClose,
  onUpdate,
  onUpload,
  ...
  const [numVisible, setNumVisible] = useState(initialNumVisible)
  ...
  useEffect(() => {
    if (isOpen) {
      // Convert initial images to editable format
      const editableImages = initialImages.map((img, index) => ({
        id: `${Date.now()}-${index}`,
        src: img.src,
        alt: img.alt || '',
        title: img.title || '',
        href: img.href || '',
        sourceType: 'url' as 'url' | 'local',
      }))
      setImages(editableImages)
      setNumVisible(initialNumVisible)
  ...
  }, [isOpen, initialImages, initialNumVisible, initialCircular])
  ...
  const handleUpdate = () => {
    ...
    onUpdate(images, numVisible, circular)
    onClose()
  }
      ...
      >
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          ...
          <div className="p-4 border border-gray-300 dark:border-gray-600 rounded-lg">
            <h4 className="text-sm font-semibold mb-3 text-gray-900 dark:text-gray-100">Add New Image</h4>
