...
interface ChartEditModalProps {
  isOpen: boolean
  onClose: () => void
  onUpdate: (chartMarkdown: string) => void
  initialType?: string
  initialData?: any
  initialOptions?: any
}
...
export const ChartEditModal: React.FC<ChartEditModalProps> = ({
  ...
  const [chartType, setChartType] = useState<ChartType>(initialType as ChartType)
  ...
  // Chart options
  const [indexAxis, setIndexAxis] = useState<'x' | 'y'>(initialOptions.indexAxis || 'x')
  const [legend, setLegend] = useState(initialOptions.legend !== false)
  const [stacked, setStacked] = useState(initialOptions.stacked || false)
  const [tension, setTension] = useState(initialOptions.tension || 0.4)
  const [fill, setFill] = useState(initialOptions.fill || false)
  const [cutout, setCutout] = useState(initialOptions.cutout || '50%')
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      // Extract dataset names and row data from initialData
      if (initialData.datasetNames && initialData.rows) {
        setDatasetNames(initialData.datasetNames)
        setRows(initialData.rows)
      }

      // Set options
      setIndexAxis(initialOptions.indexAxis || 'x')
      setLegend(initialOptions.legend !== false)
      setStacked(initialOptions.stacked || false)
      setTension(initialOptions.tension || 0.4)
      setFill(initialOptions.fill || false)
      setCutout(initialOptions.cutout || '50%')
    }
  }, [isOpen, initialType, initialData, initialOptions])
