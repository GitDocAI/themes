      ...
      case 'group':
        return (
          <div key={item.title} className={`mb-6`}>
            <div className="sidebar-text font-semibold text-xs uppercase mb-3 tracking-wider text-lupe-500 dark:text-lupe-400">
              {item.title}
            </div>
 ...
 open={isOpen}
          >
            <summary className="sidebar-text flex items-center justify-between py-2 px-3 text-sm font-medium rounded-lg hover:bg-gradient-to-r hover:from-accent-primary/5 hover:to-accent-secondary/5 hover:text-accent-primary transition-all duration-200">
              ...
              <svg className="w-4 h-4 transition-transform duration-200 group-open:rotate-90 text-lupe-500 dark:text-lupe-400" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
              </svg>
            </summary>
            <div className="mt-1 space-y-1 pl-3 border-l border-lupe-200 dark:border-lupe-700 ml-3">
              {item.children?.map(child => renderNestedItem(child as NavigationPage, 1))}
            </div>
          </details>
        )
        ...
        return item.title ? (
          <Anchor
            ...
            className={`${isActive ? '' : 'sidebar-text'} text-primary block py-2 px-3 rounded-lg text-sm transition-all duration-200 ease-in-out ${paddingLeft} ${isActive
              ? 'font-medium'
              : 'hover:bg-gradient-to-r hover:from-lupe-100/50 hover:to-lupe-200/30 dark:hover:from-lupe-800/50 dark:hover:to-lupe-700/30 dark:hover:text-lupe-100 hover:transform hover:translate-x-1'
              }`}
          >
            {item.title}
          </Anchor>
  ...
  return (
    <>
      ...
      <div className="sm:hidden py-2 border-b border-lupe-200/20 dark:border-lupe-700/20 sticky top-0 z-20 flex items-center gap-2">
      ...
      <aside
        className={`
          sidebar
          fixed sm:sticky top-24 left-0 h-full max-h-[90dvh] w-72 overflow-hidden py-6 z-50 sm:z-10
          transition-all duration-300 min-w-64 ease-in-out sm:bg-transparent
          backdrop-blur-sm
          ${isOpen ? 'translate-x-0 glass-card ml-4 rounded-xl shadow-lupe' : '-translate-x-full'} sm:translate-x-0 sm:glass-none sm:shadow-none sm:ml-0 sm:rounded-none
      ...
      >
        ...
        <nav className="space-y-1 pl-4">
          {/* SearchBar integrado en el sidebar */}
