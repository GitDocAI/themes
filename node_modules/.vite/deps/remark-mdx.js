import {
  ...
  unicodeWhitespace,
  values
} from "./chunk-NPTREH7V.js";
import {
  VFileMessage
} from "./chunk-RIXVSS6V.js";
import {
  stringifyPosition
...
function eventsToAcorn(events, options) {
  ...
  try {
    ...
    ok(point.offset !== void 0, "expected `offset`");
  ...
  if (estree) {
    ...
    if (Array.isArray(onComment)) {
      ...
      for (const comment of comments) {
        ok(comment.loc, "expected `loc` on comment");
        onComment(
          comment.type === "Block",
          comment.value,
          comment.start,
...
function factoryMdxExpression(effects, ok2, type, markerType, chunkType, acorn, acornOptions, addResult, spread, allowEmpty, allowLazy) {
  ...
  function inside(code) {
    if (code === codes.rightCurlyBrace && size === 0 || code === codes.eof || markdownLineEnding(code)) {
...
function mdxExpression(options) {
  ...
  function tokenizeFlowExpression(effects, ok2, nok) {
    ...
    function after(code) {
      return markdownSpace(code) ? factorySpace(effects, end, types.whitespace)(code) : end(code);
    }
    function end(code) {
...
function factoryTag(effects, ok2, nok, acorn, acornOptions, addResult, allowLazy, tagType, tagMarkerType, tagClosingMarkerType, tagSelfClosingMarker, tagNameType, tagNamePrimaryType, tagNameMemberMarkerType, tagNameMemberType, tagNamePrefixMarkerType, tagNameLocalType, tagExpressionAttributeType, tagExpressionAttributeMarkerType, tagExpressionAttributeValueType, tagAttributeType, tagAttributeNameType, tagAttributeNamePrimaryType, tagAttributeNamePrefixMarkerType, tagAttributeNameLocalType, tagAttributeInitializerMarkerType, tagAttributeValueLiteralType, tagAttributeValueLiteralMarkerType, tagAttributeValueLiteralValueType, tagAttributeValueExpressionType, tagAttributeValueExpressionMarkerType, tagAttributeValueExpressionValueType) {
  ...
  function nameBefore(code) {
    ...
    if (code !== codes.eof && code >= 0 && start(code)) {
    ...
    crash(
      code,
      "before name",
      "a character that can start a name, such as a letter, `$`, or `_`" + (code === codes.exclamationMark ? " (note: to create a comment in MDX, use `{/* text */}`)" : "")
    );
  }
  function closingTagNameBefore(code) {
    ...
    crash(
      code,
      "before name",
      "a character that can start a name, such as a letter, `$`, or `_`" + (code === codes.asterisk || code === codes.slash ? " (note: JS comments in JSX tags are not supported in MDX)" : "")
    );
  }
  function primaryName(code) {
    if (code !== codes.eof && code >= 0 && cont(code, { jsx: true })) {
      effects.consume(code);
      return primaryName;
    }
    if (code === codes.dot || code === codes.slash || code === codes.colon || code === codes.greaterThan || code === codes.leftCurlyBrace || markdownLineEndingOrSpace(code) || unicodeWhitespace(code)) {
  ...
  function primaryNameAfter(code) {
    ...
    if (code === codes.slash || code === codes.greaterThan || code === codes.leftCurlyBrace || code !== codes.eof && code >= 0 && start(code)) {
  ...
  function attributePrimaryNameAfter(code) {
    ...
    if (code === codes.slash || code === codes.greaterThan || code === codes.leftCurlyBrace || markdownLineEndingOrSpace(code) || unicodeWhitespace(code) || code !== codes.eof && code >= 0 && start(code)) {
      effects.exit(tagAttributeNameType);
  ...
  function attributeValueBefore(code) {
    ...
    crash(
      ...
      "a character that can start an attribute value, such as `\"`, `'`, or `{`" + (code === codes.lessThan ? " (note: to use an element or fragment as a prop value in MDX, use `{<element />}`)" : "")
    );
  }
  ...
  function tagEnd(code) {
    ok(code === codes.greaterThan, "expected `>`");
    effects.enter(tagMarkerType);
    effects.consume(code);
    effects.exit(tagMarkerType);
  ...
  function crash(code, at2, expect) {
    ...
    error.url = trouble2 + (code === codes.eof ? "#unexpected-end-of-file-at-expected-expect" : "#unexpected-character-at-expected-expect");
    throw error;
  }
}
  ...
  function tokenizeJsxFlow(effects, ok2, nok) {
    ...
    function end(code) {
      ...
      return code === codes.lessThan ? (
        ...
        start2(code)
      ) : code === codes.leftCurlyBrace && expression ? effects.attempt(expression, end, nok)(code) : code === codes.eof || markdownLineEnding(code) ? ok2(code) : nok(code);
    }
  }
}
...
var blankLineBefore = { tokenize: tokenizeNextBlank, partial: true };
...
function mdxjsEsm(options) {
  ...
  function tokenizeExportImport(effects, ok2, nok) {
    ...
    function start2(code) {
      ...
      if (self2.now().column > 1) return nok(code);
    ...
    function word(code) {
      ...
      if ((buffer === "import" || buffer === "export") && code === codes.space) {
        effects.consume(code);
        return inside;
      }
      return nok(code);
...
export {
  remarkMdx as default
};
//# sourceMappingURL=remark-mdx.js.map
