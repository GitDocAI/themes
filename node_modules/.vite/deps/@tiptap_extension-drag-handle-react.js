import {
  NodeRangeSelection,
  getSelectionRanges
} from "./chunk-UKTD7SJR.js";
import {
  require_jsx_runtime
} from "./chunk-OQYS5TZZ.js";
import {
  require_react
} from "./chunk-3MRJMIL7.js";
import {
  Extension
} from "./chunk-LSNKTKLH.js";
import {
  AllSelection,
  Fragment,
  NodeSelection,
...
var equalFlat = (a, b) => a === b || size2(a) === size2(b) && every(a, (val, key) => (val !== void 0 || hasProperty(b, key)) && b[key] === val);
...
var writeStructToLazyStructWriter = (lazyWriter, struct, offset2) => {
...
var ContentDeleted = class _ContentDeleted {
   ...
   */
  splice(offset2) {
    const right = new _ContentDeleted(this.len - offset2);
    this.len = offset2;
    return right;
  }
  /**
...
var ContentJSON = class _ContentJSON {
   ...
   */
  splice(offset2) {
    const right = new _ContentJSON(this.arr.slice(offset2));
...
var ContentAny = class _ContentAny {
   ...
   */
  splice(offset2) {
    const right = new _ContentAny(this.arr.slice(offset2));
    this.arr = this.arr.slice(0, offset2);
    return right;
  }
  /**
...
var ContentString = class _ContentString {
   ...
   */
  splice(offset2) {
    const right = new _ContentString(this.str.slice(offset2));
...
var ProsemirrorBinding = class {
  ...
  _forceRerender() {
    ...
    this.mux(() => {
      ...
      if (sel) {
        const clampedAnchor = min2(max2(sel.anchor, 0), tr.doc.content.size);
        const clampedHead = min2(max2(sel.head, 0), tr.doc.content.size);
...
// node_modules/@tiptap/extension-drag-handle/dist/index.js
function getCSSText(element2) {
  let value = "";
...
function getComputedStyle22(node, property) {
...
function getInnerCoords(view, x, y) {
  const paddingLeft = parseInt(getComputedStyle22(view.dom, "paddingLeft"), 10);
  const paddingRight = parseInt(getComputedStyle22(view.dom, "paddingRight"), 10);
  const borderLeft = parseInt(getComputedStyle22(view.dom, "borderLeftWidth"), 10);
  const borderRight = parseInt(getComputedStyle22(view.dom, "borderLeftWidth"), 10);
...
function getDragHandleRanges(event, editor) {
  const { doc: doc2 } = editor.view.state;
  ...
  const nodeAt = doc2.resolve(pos).parent;
  if (!nodeAt) {
    return [];
  }
  const $from = doc2.resolve(result.pos);
  const $to = doc2.resolve(result.pos + 1);
...
function dragHandler(event, editor) {
  ...
  const from3 = ranges[0].$from.pos;
  const to = ranges[ranges.length - 1].$to.pos;
  const selection = NodeRangeSelection.create(view.state.doc, from3, to);
  const slice = selection.content();
  ranges.forEach((range) => {
    const element2 = view.nodeDOM(range.$from.pos);
    const clonedElement = cloneElement(element2);
...
var getOuterNodePos = (doc2, pos) => {
  const resolvedPos = doc2.resolve(pos);
...
var DragHandlePlugin = ({
  ...
  function repositionDragHandle(dom) {
    ...
    computePosition2(virtualElement, element2, computePositionConfig).then((val) => {
  ...
  element2.addEventListener("dragstart", onDragStart);
  element2.addEventListener("dragend", onDragEnd);
  wrapper.appendChild(element2);
  return {
    unbind() {
      element2.removeEventListener("dragstart", onDragStart);
      element2.removeEventListener("dragend", onDragEnd);
      if (rafId) {
    ...
    plugin: new Plugin({
      ...
      state: {
        ...
        apply(tr, value, _oldState, state) {
          ...
          if (tr.docChanged && currentNodePos !== -1 && element2) {
...
var DragHandle = Extension.create({
  ...
  addOptions() {
    return {
      render() {
        ...
        element2.classList.add("drag-handle");
        return element2;
      },
      computePositionConfig: {},
      locked: false,
  ...
  addProseMirrorPlugins() {
    const element2 = this.options.render();
    return [
      DragHandlePlugin({
...
// node_modules/@tiptap/extension-drag-handle-react/dist/index.js
var import_react = __toESM(require_react());
var import_jsx_runtime = __toESM(require_jsx_runtime());
var DragHandle2 = (props) => {
  const {
  ...
  const [element2, setElement] = (0, import_react.useState)(null);
  ...
  }, [element2, editor, onNodeChange, pluginKey, computePositionConfig, onElementDragStart, onElementDragEnd]);
  return (0, import_jsx_runtime.jsx)("div", { className, style: { visibility: "hidden", position: "absolute" }, ref: setElement, children });
};
var index_default = DragHandle2;
export {
  DragHandle2 as DragHandle,
  index_default as default
};
//# sourceMappingURL=@tiptap_extension-drag-handle-react.js.map
