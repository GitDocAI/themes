...
import { ApiSchema } from "../../../models/ApiReference.models";



export const ApiSchemaRenderer =(
  {
  propName,
  propSchema,
  requestBodySchema
      ...
      constraints.push(
        <span key="enum" className="flex flex-wrap">Available Options:
            {schema.enum.map((option:string)=>(
                <span key={option} className="rounded-md bg-secondary/5 px-2 py-0.5 text-xs text-secondary ml-1">
                    {option}
                </span>
            ))}
        </span>
  ...
  const renderProperty = (
    ...
    const isRequired = requiredFields?.includes(name) || schema.required;
