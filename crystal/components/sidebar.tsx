...
export const Sidebar = ({ themeinfo, versions, tabs }: { themeinfo: ThemeInfo, versions: Version[], tabs: Tab[] }) => {
  ...
  const methodColors: Record<string, string> = {
    ...
    POST: "bg-gradient-to-r from-blue-400 to-blue-500 text-white shadow-blue-200 dark:shadow-blue-800/30",
    PUT: "bg-gradient-to-r from-amber-400 to-amber-500 text-white shadow-amber-200 dark:shadow-amber-800/30",
    DELETE: "bg-gradient-to-r from-rose-400 to-rose-500 text-white shadow-rose-200 dark:shadow-rose-800/30",
  };
...
function dropdownHasActiveChild(dropdown: NavigationItem, pathname: string): boolean {
 ...
 open={isOpen}
          >
            ...
            <div className="mt-2 space-y-1 pl-4 border-l-2 border-gradient-to-b border-crystal-200/30 ml-3">
              {item.children?.map(child => renderNestedItem(child as NavigationPage, 1))}
            </div>
          </details>
        )
