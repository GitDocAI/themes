...
const FormContext = React.createContext<Record<string, any>>(
  {}
);
  ...
  return (
    <Controller
      ...
      render={({ field, fieldState, formState }) => (
        <FormContext.Provider value={{ name: props.name, field, fieldState, formState }}>
...
const useFormField = () => {
  ...
  const { name, id } = itemContext;

  if (!fieldContext || !name) {
    throw new Error("`useFormField` should be used within `<FormField>`");
  }

  const { field, fieldState, formState } = fieldContext;
