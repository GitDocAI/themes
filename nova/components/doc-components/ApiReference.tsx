  ...
  return (
    <div className="
      ...
      <div id="apiref" className="rounded-2xl p-6 transition [grid-area:reference] w-full">
        ...
        {tags?.length ? (
          <div className="flex gap-2 mb-4 flex-wrap">
            {tags.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1.5 text-xs rounded-lg bg-nova-100 dark:bg-nova-700/40 text-nova-700 dark:text-nova-200 border border-nova-200 dark:border-nova-600/40 shadow-sm"
              >
                {tag}
              </span>
            ))}
        ...
        {(path || method) && (
          <div className="flex items-center gap-4 mb-6 mt-4 w-full border-nova-300 dark:border-nova-600/30 border py-4 px-5 rounded-xl bg-nova-50/50 dark:bg-nova-800/20 shadow-sm">
            {method && (
              <span
            ...
            {path && (
              <div className="flex-1 border-nova-300 dark:border-nova-600/30 border py-3 px-4 rounded-lg hover:bg-nova-50 dark:hover:bg-nova-700/40 hover:border-accent-primary/40 dark:hover:border-accent-primary/40 transition-all duration-200 cursor-pointer shadow-sm">
                <CodeBlock>
                  <span className="text-sm font-mono text-nova-800 dark:text-nova-100">{path}</span>
                </CodeBlock>
              </div>
            )}
            {tryItBaseUrl && (
